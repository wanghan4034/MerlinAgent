...
_RECOMMEND_RULES: list[dict[str, Any]] = [
    {
        "name": "CHANEL 包包高性价比",
        "brands": ["chanel", "香奈儿", "香奈爾"],
        "categories": ["bag", "包", "手袋", "ショルダー", "トート", "バッグ"],
        "max_price": 120000,
        "advice": "优先看磨损描述清晰、五金细节图完整的低价 CHANEL 包。",
    },
    {
        "name": "爱马仕包包价格洼地",
        "brands": ["hermes", "爱马仕", "エルメス"],
        "categories": ["bag", "包", "手袋", "バッグ"],
        "max_price": 220000,
        "advice": "关注皮质保养情况与边角磨损，优先选择近30天新上架且价格低于阈值的货。",
    },
    {
        "name": "服饰日常捡漏",
        "brands": [],
        "categories": ["clothes", "衣服", "外套", "上衣", "裙", "裤", "シャツ", "ジャケット", "コート"],
        "max_price": 15000,
        "advice": "二手服饰优先挑有尺码与实拍细节图、价格明显低于同类均值的商品。",
    },
]


def _recommendation_for_item(row: dict[str, Any]) -> dict[str, Any]:
    text = f"{row.get('title') or ''} {row.get('keyword') or ''}".lower()
    price = row.get("price_jpy")
    is_sold = bool(row.get("is_sold"))
    if is_sold or price is None:
        return {"is_recommended": False, "reason": "", "rule": "", "score": 0}

    for rule in _RECOMMEND_RULES:
        brand_ok = True if not rule["brands"] else any(token.lower() in text for token in rule["brands"])
        category_ok = any(token.lower() in text for token in rule["categories"])
        if not (brand_ok and category_ok):
            continue
        if int(price) > int(rule["max_price"]):
            continue

        score = max(1, min(100, int((rule["max_price"] - int(price)) / max(rule["max_price"], 1) * 100)))
        return {
            "is_recommended": True,
            "rule": rule["name"],
            "reason": f"{rule['advice']} | 当前价 ¥{price} <= 阈值 ¥{rule['max_price']}",
            "score": score,
        }
...
def _parse_keywords(value: str | list[str]) -> list[str]:
    ...
    if "\n" in text or ";" in text or "；" in text:
        ...
        for line in text.splitlines():
            parts.extend(chunk.strip() for chunk in line.replace("；", ";").split(";"))
...
def _fetch_recent_items(
    ...
    *,
    ...
    try:
        ...
        for row in data:
            row.update(_recommendation_for_item(row))
        if recommended_only:
            data = [row for row in data if row.get("is_recommended")]
        return data
    finally:
        conn.close()
...
def api_items():
    ...
    recommended_only = request.args.get("recommended_only", "0") == "1"
